/**
 * Gap Feature: gap-no-approval-workflow
 * No approval workflow.
 *
 * POST /api/gap-no-approval-workflow
 * Auth required. Generated as part of Gaps backend scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_GAP_NO_APPROVAL_WORKFLOW_KEY
...
const FEATURE_KEY = process.env.FEATURE_GAP_NO_APPROVAL_WORKFLOW_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: No approval workflow..\nGoal: No approval workflow.\nRespond with clear, actionable analysis. Prefer JSON when structured output is requested.`;
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiContentStudio - gap-no-approval-workflow',
    },
    body: JSON.stringify({
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_GAP_NO_APPROVAL_WORKFLOW_KEY');
    const ai = await callLLM({ feature: 'gap-no-approval-workflow', goal: 'No approval workflow.', input: payload });
    try {
      ...
      if (pool && pool.query) {
        await pool.query(
          ...
          ['gap-no-approval-workflow', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[gap-no-approval-workflow] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'gap-no-approval-workflow',
      endpoint: '/api/gap-no-approval-workflow',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[gap-no-approval-workflow] error:', err.message);
